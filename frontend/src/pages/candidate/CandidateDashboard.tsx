  ...
  return (
    <div className={`rounded-2xl border p-5 flex items-center gap-4 ${color}`}>
      ...
      <div>
        ...
        <p className="text-sm text-gray-900 dark:text-white/60 mt-0.5">{label}</p>
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-white dark:bg-slate-900 border border-black/10 dark:border-white/10 rounded-2xl shadow-[0_25px_60px_rgba(0,0,0,0.5)] max-w-lg w-full max-h-[85vh] flex flex-col overflow-hidden"
      >
        {/* ── Header ── */}
        <div className="p-6 border-b border-black/10 dark:border-white/10 shrink-0">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">{offre.titre}</h2>
            </div>
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-lg bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:bg-white/10 flex items-center justify-center text-gray-600 dark:text-slate-400 hover:text-gray-900 dark:text-white transition-colors shrink-0"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          {/* Description */}
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-slate-500 uppercase tracking-widest mb-2">Description du poste</p>
            <p className="text-sm text-gray-600 dark:text-slate-300 leading-relaxed">{offre.description}</p>
          </div>

          {/* Méta */}
          <div className="flex flex-wrap gap-4 text-sm text-gray-600 dark:text-slate-400">
            {offre.annees_experience_min > 0 && (
              <span className="flex items-center gap-1.5 bg-black/5 dark:bg-white/5 px-3 py-1.5 rounded-lg border border-black/10 dark:border-white/10">
          ...
          {offre.competences_requises.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-500 dark:text-slate-500 uppercase tracking-widest mb-2">Compétences requises</p>
          ...
          <div className="border-t border-black/10 dark:border-white/10 pt-5">
            {dejaPostule && candidature ? (
              ...
              <div className="space-y-3">
                <p className="text-xs font-semibold text-gray-500 dark:text-slate-500 uppercase tracking-widest">Ma candidature</p>

                <div className="bg-black/5 dark:bg-white/5 rounded-xl border border-black/10 dark:border-white/10 p-4 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-blue-500/10 flex items-center justify-center shrink-0">
                    <FileText className="w-5 h-5 text-blue-300" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 dark:text-white">CV soumis</p>
                    <p className="text-xs text-gray-600 dark:text-slate-400 truncate mt-0.5">{candidature.cv_nom_fichier}</p>
                    <p className="text-xs text-gray-500 dark:text-slate-600 mt-0.5">
              ...
              <div className="space-y-3">
                <p className="text-sm font-medium text-gray-900 dark:text-white">
                  Téléversez votre CV{" "}
                  <span className="text-gray-500 dark:text-slate-500 font-normal">(PDF, DOCX, DOC — 5 Mo max)</span>
                </p>
                <label className={`block w-full border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all duration-200 ${cvFile
                  ...
                  : "border-black/10 dark:border-gray-200 dark:border-white/10 hover:border-blue-500/40 hover:bg-blue-500/5"
                  }`}>
                  ...
                  {cvFile ? (
                    ...
                    <div className="text-gray-500 dark:text-slate-500">
                      <Upload className="w-7 h-7 mx-auto mb-2 text-gray-500 dark:text-slate-600" />
        ...
        {/* ── Footer actions ── */}
        <div className="p-5 border-t border-black/10 dark:border-white/10 shrink-0 flex gap-3">
          ...
          {!dejaPostule && (
            <button
              ...
              className="flex-1 py-2.5 rounded-xl bg-blue-600 text-gray-900 dark:text-white text-sm font-semibold hover:bg-blue-500 transition-all shadow-md hover:shadow-[0_4px_20px_rgba(37,99,235,0.4)] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {postuleLoading ? (
    ...
    <DashboardLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-600 dark:text-slate-400 text-sm mt-1">Découvrez les offres et suivez vos candidatures.</p>
        </div>
            ...
            >
                ...
                {DOMAINES.map((d) => (
                  <button key={d} onClick={() => setFilterDomaine(d)}
                    className={`px-3.5 py-1.5 rounded-full text-sm font-medium transition-all duration-200 border ${filterDomaine === d
                      ? "bg-blue-600 text-gray-900 dark:text-white border-blue-600 shadow-md"
                      : "bg-black/5 dark:bg-white/5 text-gray-600 dark:text-slate-300 border-black/10 dark:border-white/10 hover:bg-black/10 dark:bg-white/10 hover:border-black/20 dark:border-white/20"
                      }`}>
                    {d}
                  </button>
                ))}
              ...
              {loading ? (
                ...
                <div className="text-center py-20">
                  ...
                  <p className="text-gray-600 dark:text-slate-400">Aucune offre disponible pour ce domaine.</p>
                </div>
              ) : (
                  ...
                  {offres.map((offre, i) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="group bg-black/5 dark:bg-white/5 hover:bg-black/8 dark:bg-white/8 border border-black/10 dark:border-white/10 hover:border-blue-500/40 rounded-2xl p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_40px_rgba(37,99,235,0.1)] flex flex-col"
                      >
                        {/* Header carte */}
                        ...
                        {/* Titre & Description */}
                        <h3 className="font-bold text-gray-900 dark:text-white text-base mb-2 line-clamp-2 group-hover:text-blue-200 transition-colors">
                          {offre.titre}
                        </h3>
                        <p className="text-gray-600 dark:text-slate-400 text-sm line-clamp-3 mb-4 leading-relaxed flex-1">{offre.description}</p>

                        {/* Méta */}
                        <div className="flex flex-wrap gap-3 text-xs text-gray-500 dark:text-slate-500 mb-4">
                          {offre.annees_experience_min > 0 && (
                        ...
                        {offre.competences_requises.length > 0 && (
                          <div className="flex flex-wrap gap-1.5 mb-4">
                            {offre.competences_requises.slice(0, 3).map((c) => (
                              <span key={c} className="px-2 py-0.5 rounded-md text-xs bg-black/5 dark:bg-white/5 text-gray-600 dark:text-slate-400 border border-black/10 dark:border-white/10">
                                {c}
                              </span>
                            ))}
                        ...
                        <button
                          ...
                          className={`w-full flex items-center justify-center gap-2 py-2.5 text-sm font-semibold rounded-xl transition-all active:scale-[0.98] ${dejaPostule
                            ? "bg-black/5 dark:bg-white/5 text-gray-600 dark:text-slate-300 border border-black/10 dark:border-white/10 hover:bg-black/10 dark:bg-white/10 hover:border-black/20 dark:border-white/20"
                            : "bg-blue-600 text-gray-900 dark:text-white hover:bg-blue-500 hover:shadow-[0_4px_20px_rgba(37,99,235,0.4)]"
                            }`}
                        >
                          {dejaPostule ? (
            ...
            >
              {candidatures.length === 0 ? (
                <div className="text-center py-20 bg-black/5 dark:bg-white/5 rounded-2xl border border-black/10 dark:border-white/10">
                  <ClipboardList className="w-10 h-10 text-gray-500 dark:text-slate-600 mx-auto mb-3" />
                  ...
                  {candidatures.map((c, i) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl p-5 flex items-center gap-4 hover:border-black/20 dark:border-white/20 hover:bg-black/7 dark:bg-white/7 transition-all duration-200 group"
                      >
                        {/* Icône offre */}
                        ...
                        {/* Infos */}
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-gray-900 dark:text-white text-sm truncate">{c.offre_titre ?? "Offre"}</p>
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          ...
                          <button
                            ...
                            className="opacity-0 group-hover:opacity-100 w-8 h-8 rounded-lg bg-black/5 dark:bg-white/5 hover:bg-blue-500/20 border border-black/10 dark:border-white/10 hover:border-blue-500/30 flex items-center justify-center text-gray-600 dark:text-slate-400 hover:text-blue-300 transition-all"
                          >
